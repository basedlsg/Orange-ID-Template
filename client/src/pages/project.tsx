...
export default function ProjectPage() {
  ...
  const { data: project, isLoading } = useQuery<Project>({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/projects/by-slug/${slug.split("?")[0]}`,
      );
      if (!response.ok) {
  ...
  const thumbnailUrl =
    project.thumbnail || `${window.location.origin}/default-thumbnail.png`;
  ...
  return (
    <div className="min-h-screen bg-black">
      <Helmet>
        ...
        <meta
          property="og:title"
          content={`${project.name} - VibeCodingList`}
        />
        ...
        <link
          ...
          href={`${window.location.origin}/meta-preview/${slug}`}
          title={`${project.name} - VibeCodingList`}
        />
        <link rel="canonical" href={window.location.href} />
        ...
        <meta
          name="keywords"
          content={`${project.genres.join(", ")}, ${project.aiTools?.join(", ")}, AI Projects, Coding Projects`}
        />
      </Helmet>
